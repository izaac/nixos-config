...
```bash
nix run github:nix-community/disko -- --mode dry-run github:izaac/nixos-config#ninja
```

### Execution
Once verified, run the formatting and partitioning logic. **Warning: This will destroy all data on the target disks.**
```bash
nix run github:nix-community/disko -- --mode disko --flake github:izaac/nixos-config#ninja
```
This command will:
...
## Why this works
The `hardware.nix` file contains a conditional "shield" (`lib.mkIf (!config.disko.enable)`). During a standard build, `disko.enable` is false, so `hardware.nix` provides the mount points and LUKS info. During a `disko` operation, `disko` takes over, providing identical mount points and ensuring the system stays in sync.
