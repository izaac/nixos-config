...
`swayidle` locks after 6 minutes idle and powers the displays off at 11; both
timers run from the start of idle, so the lock screen stays lit for 5 minutes.
Any input wakes the displays. Suspend is never automatic. The screen also locks
before sleep and on `loginctl lock-session`. Manual lock is `Mod+Ctrl+L`, and
unlock takes a YubiKey touch or your password.

The lock screen shows the current wallpaper, blurred and vignetted, with a clock
in the indicator. `screenshots` mode is avoided on purpose: it renders the real
desktop, and blur is obfuscation rather than concealment. `--grace` is unset for
the same reason, since it unlocks on any input for N seconds with no password.
