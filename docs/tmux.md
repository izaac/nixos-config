...
## Session layouts (smug)

[smug](https://github.com/ivaaaan/smug) builds prearranged sessions from YAML. It is a single Go
binary, so unlike `tmuxinator` (Ruby) or `tmuxp` (Python) it drags in no language runtime.

The config is generated by Nix in `home/smug.nix` rather than hand-written, so the layout stays
declarative and the window list cannot drift from what is committed.

```bash
smug start daily     # create the session (or attach if it already exists)
smug stop daily      # kill it
smug list            # available layouts
smug print daily     # dump the config
```
...
panes, and two entries would produce three.
