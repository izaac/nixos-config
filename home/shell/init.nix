_: {
  programs.bash.initExtra = ''
    ...
    # Drop every interactive SSH login straight into a long-lived `trip` tmux
    # session so flaky cellular/hotel WiFi can reconnect into the same shell
    # state. `new-session -A` attaches if it exists, creates it otherwise.
    ...
    if [[ $- == *i* && -n "''${SSH_TTY-}" && -z "''${TMUX-}" && -z "''${VSCODE_INJECTION-}" ]]; then
      exec tmux new-session -A -s trip
    fi
  '';
}
