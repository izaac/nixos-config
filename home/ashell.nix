{
  ...
  ashellTemplate = (pkgs.formats.toml {}).generate "ashell-template.toml" {
    ...
    settings =
      {
        lock_cmd = "${lib.getExe pkgs.playerctl} --all-players pause; ${lib.getExe pkgs.swaylock-effects} -f";
        suspend_cmd = "systemctl suspend";
        reboot_cmd = "systemctl reboot";
        shutdown_cmd = "systemctl poweroff";
        logout_cmd = "${lib.getExe niri} msg action quit --skip-confirmation";
        audio_sinks_more_cmd = "${lib.getExe pkgs.pulsemixer}";
        audio_sources_more_cmd = "${lib.getExe pkgs.pulsemixer}";
  ...
  # Separate script so build_thumbs can hand pairs straight to xargs without
  # an inline `sh -c`, which shellcheck flags for unexpanded quoting.
  magickThumb = pkgs.writeShellApplication {
    name = "magick-thumb";
    runtimeInputs = [pkgs.imagemagick];
    text = ''
      magick "$1" -thumbnail 256x256 "$2" 2>/dev/null || true
    '';
  };

  setWallpaper = pkgs.writeShellApplication {
    name = "set-wallpaper";
    runtimeInputs = [pkgs.awww pkgs.matugen pkgs.fuzzel pkgs.coreutils pkgs.findutils pkgs.libnotify magickThumb];
    text = ''
      list_in() {
      ...
      thumb_path() {
        printf '%s/%s.png' "${thumbDir}" "$(printf '%s' "$1" | sha1sum | cut -c1-32)"
      }

      # fuzzel renders icons through libpng and libresvg only, with no libjpeg,
      # so most of the collection cannot be previewed directly. Cache a small
      # PNG per image instead; the originals stay untouched, which matters
      # because the wallpaper directory is somebody else's git checkout.
      build_thumbs() {
        mkdir -p "${thumbDir}"
        local src thumb
  ...
  clipboardMenu = pkgs.writeShellApplication {
    ...
    text = ''
      sel=$(stash list | fuzzel --dmenu --prompt "Clipboard: ") || exit 0
      [ -n "$sel" ] || exit 0
...
in {
  ...
  programs.niri.settings = {
    ...
    # portal captures; the bar, menus and OSD are harmless.
    layer-rules = [
      {
        matches = [{namespace = "^ashell-toast-layer";}];
